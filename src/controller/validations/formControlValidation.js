...
module.exports = {
  ...
  fetchAPIData: async (req, res) => {
    const { spName, filterCondition: rawCondition = "{}", clientId } = req.body;

    if (!spName || typeof spName !== "string") {
      return res
        .status(400)
        .json({ success: false, message: "Must provide a valid spName" });
    }

    // Build the single JSON param your proc expects
    let conditionObj;
    try {
      conditionObj =
        typeof rawCondition === "string"
          ? JSON.parse(rawCondition)
          : rawCondition;
    } catch {
      return res
        .status(400)
        .json({ success: false, message: "filterCondition is not valid JSON" });
    }

    // Include any other params (e.g. clientId) in your one JSON payload
    const filterCondition = { ...conditionObj, clientId };

    try {
      const data = await execSpWithJsonParam(spName, filterCondition);

      if (!Array.isArray(data)) {
        return res.status(500).json({
          success: false,
          message: "Stored procedure did not return an array",
          data,
        });
      }
      ...
      console.error(`Error fetching [${spName}]:`, err);
      return res.status(500).json({
        success: false,
        message: `Failed to execute ${spName}`,
        error: err.message,
      });
    }
  },
  ...
  insertReportData: async (req, res) => {
    const { spName, filterCondition } = req.body;
    if (!spName) {
      return res
        .status(400)
        .json({ success: false, message: "spName is required" });
    }
    try {
      // now returns something like [5]
      const rowsAffected = await executeNonQueryStoredProcedure(
        spName,
        filterCondition
      );
      ...
      res.status(200).json({
        success: true,
        message: `${rowsAffected} inserted successfully.`,
        rowsAffected,
      });
    } catch (err) {
      console.error(`Insert SP error:`, err.message);
      res.status(500).json({
        success: false,
        message: `Failed to execute insert SP "${spName}". and the error is - ${err.message}`,
        error: err.message, // ← this will include your T-SQL ERROR_MESSAGE()
      });
    }
  },
};
